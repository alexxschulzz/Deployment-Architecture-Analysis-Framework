...
function parseArchitectureXML(channel, xmlContent, fileName, architectureName, origFormat, originalMsgContent, originalMsg) {
    ...
    parser.parseString(xmlContent, (err, result) => {
        ...
        const architecture = result['archimate:model'];
        const folders = architecture.folder || [];
        const extractedData = [];
        const elementsById = {};

        // Find and save the elements from the ‘Application’ and ‘Technology & Physical’ folders
        folders.forEach(folder => {
            const folderName = folder.$.name;

            if (folderName === 'Application' || folderName === 'Technology & Physical') {
        ...
        // Checking the relationships in the ‘Relations’ folder
        const relationsFolder = folders.find(folder => folder.$.name === 'Relations');
        if (relationsFolder) {
            const relationships = relationsFolder.element || [];
            relationships.forEach(rel => {
                const sourceId = rel.$.source;
                const targetId = rel.$.target;

                const sourceNode = elementsById[sourceId];
                const targetNode = elementsById[targetId];

                // Only save if both source and target are found
                if (sourceNode && targetNode) {
                    extractedData.push({
                        source_component: sourceNode.name,
                        source_component_type: sourceNode.type.replace("archimate:", ""),
                        target_component: targetNode.name,
                        target_component_type: targetNode.type.replace("archimate:", ""),
                        relationship: rel.$['xsi:type'].replace("archimate:", ""),
        ...
        if (extractedData.length === 0) {
            console.warn(" [!] No data extracted for architecture: '%s'", architectureName);
        }

        const csvFileName = fileName.replace('.archimate', '_EDMM.csv');
        writeDataToCSV(extractedData, csvFileName, () => {
            uploadCSVToMinIO(csvFileName, channel, originalMsgContent, architectureName, origFormat, originalMsg);
        });
    });
}
...
function saveCSVFileNameToMySQL(csvFileName, architectureName, origFormat, callback) {
    // First, fetch the architecture ID, user, nameofarchitecture and analysisflag from the Architectures table
